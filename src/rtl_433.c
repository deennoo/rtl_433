...
static int silverws_callback(uint8_t bb[BITBUF_ROWS][BITBUF_COLS]) {
    ...
    if (bb[1][0] == bb[5][0] && bb[2][0] == bb[6][0] && (bb[1][4] & 0xf) == 0  && (bb[5][4] & 0xf) == 0
	  ...
	  temp = (int16_t)((uint16_t)(reverse8(bb[1][1]) >> 4) | (reverse8(bb[1][2])<<4));
          ...
          fprintf(stderr, "Sensor temperature event:\n");
          fprintf(stderr, "Sonde         = Otio SHT10 - SHT20\n");
...
*/
	...
	(bb[i][1] != 0 || bb[i][2] != 0 || bb[i][3] != 0 || bb[i][4] != 0) &&
        ...
        fprintf(stderr, "Sensor temperature event:\n");
        fprintf(stderr, "Sonde         = Auriol Z32171A-TX\n");
        fprintf(stderr, "trend         = %s\n",temp_states[(bb[i][1])&0x3]);
        fprintf(stderr, "button        = no\n");
        fprintf(stderr, "battery       = %s\n",bb[i][1]&0x80?"OK":"Low");
        fprintf(stderr, "temp          = %s%d.%d\n",temp<0?"-":"",temperature_before_dec, temperature_after_dec);
        if (humidity != 0) { fprintf(stderr, "humidity      = %d\n", humidity); }
        fprintf(stderr, "channel       = %d\n", bb[i][4] & 0x3);
...
static int aureol_callback(uint8_t bb[BITBUF_ROWS][BITBUF_COLS]) {
    int temperature_before_dec;
    int temperature_after_dec;
    int16_t temp;
    int sensor_id;
    ...
    if ( bb[1][4] == 0 && *notnul != 0 && bb[0][0] == 0 && bb[1][0] != 0 &&
        ...
        //fprintf(stderr, "Sensor temperature event:\n");
        //fprintf(stderr, "sonde         = Auriol Z31443A-TX\n");
        //fprintf(stderr, "trend         = na\n");
        //fprintf(stderr, "button        = no\n");
        //fprintf(stderr, "battery       = na\n");
        //fprintf(stderr, "temp          = %s%d.%d\n",temp<0?"-":"", temperature_before_dec, temperature_after_dec);
        //fprintf(stderr, "humidity      = na\n");
        //fprintf(stderr, "channel       = 1\n");
        //fprintf(stderr, "rid           = %x(%d)\n");
...
static int oregon_scientific_v2_1_parser(uint8_t bb[BITBUF_ROWS][BITBUF_COLS]) {
   ...
   if ( ((bb[0][1] == 0x55) && (bb[0][2] == 0x55)) ||
	...
	if ((sensor_id == 0x1d20) || (sensor_id == 0x1d30))	{
	   ...
	   if (validate_os_v2_message(msg, 185, num_valid_v2_bits, 19) == 0) {
	     ...
	     fprintf(stderr,"Weather Sensor BHTR968  Indoor    Temp: %3.1fÂ°C  %3.1fÂ°F   Humidity: %d%%", temp_c, ((temp_c*9)/5)+32, get_os_humidity(msg, 0x5d60));  
 ...
 return 1;  
	   ...
	   if (validate_os_v2_message(msg, 185, num_valid_v2_bits, 19) == 0) {
	     ...
	     fprintf(stderr, " (%s) Pressure: %dmbar (%s)\n", comfort_str, ((msg[7] & 0x0f) | (msg[8] & 0xf0))+814, forecast_str);  
	   }
	   return 1;
	   ...
	   return 1;
		...
		if (  validate_os_v2_message(msg, 153, num_valid_v2_bits, 12) == 0) {
			...
			fprintf(stderr, "Temp: %3.1fÂ°C  %3.1fÂ°F\n", temp_c, ((temp_c*9)/5)+32);
		}
		return 1;
...
//fprintf(stderr,"i=%d j=%d dest_bit=%02x bb=%02x msg=%02x\n",i, j, dest_bit, bb[0][i], msg[dest_bit>>3]); 
	...
	if ((msg[0] == 0xf8) && (msg[1] == 0x24))	{
	   if (validate_os_checksum(msg, 15) == 0) {
		 ...
		 fprintf(stderr,"Weather Sensor THGR810  Channel %d Temp: %3.1fÂ°C  %3.1fÂ°F   Humidity: %d%%\n", channel, temp_c, ((temp_c*9)/5)+32, humidity);
	   }
	   return 1;
...
r_device aureol = {
    /* .id             = */ 1,
    /* .name           = */ "Auriol Temperature Sensor / Z31741A-TX",
    /* .modulation     = */ OOK_PWM_D,
    /* .short_limit    = */ 750,
    /* .long_limit     = */ 1500,
    /* .reset_limit    = */ 5000,
    /* .json_callback  = */ &aureol_callback,
};
...
r_device silver_ws = {
    /* .id             = */ 12,
    /* .name           = */ "Silvercrest Weather Sensor (2008) & OTIO : SHT10 / SHT20 "  ,
    /* .modulation     = */ OOK_PWM_D,
    /* .short_limit    = */ 3500/4,
    /* .long_limit     = */ 7000/4,
    /* .reset_limit    = */ 15000/4,
...
r_device auriol_2013b = {
    /* .id             = */ 14,
    /* .name           = */ "Auriol Weather Sensor (12/2013) / Z32171A-TX",
    /* .modulation     = */ OOK_PWM_D,
    /* .short_limit    = */ 800,
...
r_device oregon_scientific = {
    /* .id             = */ 18,
    /* .name           = */ "Oregon Scientific Weather Sensor / THGR968 / BHTR968/ BTHGN129 / RGR968 / THR228N / THN132N",
...
int main(int argc, char **argv)
{
   ...
   register_protocol(demod, &aureol);
